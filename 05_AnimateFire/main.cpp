...
osgParticle::ParticleSystem* createFireParticles(osg::Group* parent)
{
	...
	ps->getDefaultParticleTemplate().setSizeRange(osgParticle::rangef(1.5f, 0.75f));
	ps->getDefaultParticleTemplate().setAlphaRange(osgParticle::rangef(1.0f, 0.0f));
	ps->getDefaultParticleTemplate().setColorRange(
		osgParticle::rangev4(osg::Vec4(0.4f, 0.4f, 0.9f, 1.0f), osg::Vec4(0.4f, 0.4f, 0.9f, 1.0f)));
	...
	rrc->setRateRange(60, 100);

	osg::ref_ptr<osgParticle::RadialShooter> shooter = new osgParticle::RadialShooter;
	shooter->setThetaRange(-osg::PI_4*0.1, osg::PI_4*0.1);
	shooter->setPhiRange(-osg::PI_4*0.1, osg::PI_4*0.1);
	shooter->setInitialSpeedRange(20.0f, 27.5f);
...
{
	osg::ref_ptr<osgParticle::ParticleSystem> ps = new osgParticle::ParticleSystem;
	ps->getDefaultParticleTemplate().setLifeTime(1.0f);
	ps->getDefaultParticleTemplate().setShape(osgParticle::Particle::QUAD);
	ps->getDefaultParticleTemplate().setSizeRange(osgParticle::rangef(1.0f, 12.0f));
	ps->getDefaultParticleTemplate().setAlphaRange(osgParticle::rangef(1.0f, 0.0f));
	ps->getDefaultParticleTemplate().setColorRange(
		osgParticle::rangev4(osg::Vec4(0.4f, 0.4f, 0.8f, 1.0f), osg::Vec4(0.0f, 0.0f, 0.1f, 0.5f)));
	ps->setDefaultAttributes("Images/smoke.rgb", true, false);
	...
	shooter->setInitialSpeedRange(20.0f, 30.0f);
...
int main(int argc, char** argv)
{
	...
	parent->setMatrix(osg::Matrix::rotate(-osg::PI_2, osg::X_AXIS) *
		osg::Matrix::translate(0.0f, 15.0f, -3.0f));
