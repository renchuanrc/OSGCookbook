...
{
public:
	void doUserOperations(const osgUtil::LineSegmentIntersector::Intersection& result)
	{
		osg::ShapeDrawable* shape =
			dynamic_cast<osg::ShapeDrawable*>(result.drawable.get());
		if (shape)
...
int main(int argc, char** argv)
{
	...
	root->addChild(createMatrixTransform(geode1.get(), osg::Vec3(0.0, 0.0, 0.0)));
	root->addChild(createMatrixTransform(geode2.get(), osg::Vec3(-2.0, 0.0, 0.0)));
	root->addChild(createMatrixTransform(geode3.get(), osg::Vec3(2.0, 0.0, 0.0)));
